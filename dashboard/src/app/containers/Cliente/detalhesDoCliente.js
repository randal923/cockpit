...
class DetalhesDoCliente extends React.Component {
  generateStateCliente = (props) => ({
    ...
    sobreNome: props.cliente ? props.cliente.sobreNome : '',
  ...
  validate() {
    ...
    if (!sobreNome) erros.sobreNome = 'Preencha aqui com o sobre nome do cliente'
  ...
  salvarCliente() {
    ...
    this.props.updateCliente(this.state, cliente._id, (error) => {
      this.setState({
        aviso: {
          status: !error,
  ...
  renderDetalhesCadastro() {
    const { nome, sobreNome, CPF, email, telefone, dataDeNascimento, erros } = this.state
    return (
        ...
        <TextoDados
          ...
          valor={
            <InputValor
              ...
              handleSubmit={(valor) => this.handleSubmit('sobreNome', valor)}
              value={sobreNome}
            />
          }
        />
